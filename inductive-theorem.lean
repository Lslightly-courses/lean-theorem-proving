...
theorem Weekday.nextOfPrevious (d : Weekday) : next (previous d) = d :=
  ...
  | thursday  => rfl
  | friday    => rfl
  | saturday  => rfl -- rlf reflective

-- You can automate definitions such as `Weekday.nextOfPrevious`
-- using metaprogramming (or "tactics").
theorem Weekday.nextOfPrevious' (d : Weekday) : next (previous d) = d := by -- constructs a term of the expected type by running the tactic(s) tac.
